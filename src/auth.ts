export type User = { id: number; email: string; name: string; profileImage?: string | null };
...
export function createAuthClient(baseUrl: string, request: typeof fetch = fetch) {
  ...
  const refresh = (): Promise<void> => {
    ...
    refreshTask = (async () => {
      const response = await request(`${base}/api/auth/refresh`, { method: 'POST', credentials: 'include' });
      if (!response.ok) {
  ...
  return {
    ...
    loginUrl: `${base}/api/auth/login/google`,
    async logout() {
      generation++;
      accessToken = null;
      // Wait for an in-flight refresh before clearing its newly rotated cookie.
      await refreshTask?.catch(() => undefined);
      const response = await request(`${base}/api/auth/logout`, { method: 'POST', credentials: 'include' });
