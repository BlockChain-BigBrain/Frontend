...
test('cookie restore, Bearer header and shared parallel refresh',async()=>{
 ...
 const client=createAuthClient('http://localhost:3000',async(url,init)=>{
  ...
  if(String(url).endsWith('/refresh')){refreshes++;await new Promise(r=>setTimeout(r,10));return json({result:{accessToken:'access'}});}
...
test('401 retry is bounded',async()=>{
 let calls=0;const client=createAuthClient('http://localhost:3000',async(url)=>{calls++;return String(url).endsWith('/refresh')?json({result:{accessToken:'a'}}):json({},401);});
...
test('logout waits for refresh and discards late token',async()=>{
 const calls:string[]=[];const client=createAuthClient('http://localhost:3000',async(url)=>{
  calls.push(String(url));if(String(url).endsWith('/refresh')){await new Promise(r=>setTimeout(r,10));return json({result:{accessToken:'a'}});}return new Response(null,{status:204});
 });
